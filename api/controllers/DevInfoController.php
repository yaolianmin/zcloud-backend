...
{
    public $modelClass = 'backend\models\DevInfo';//指定调用这个控制器时链接哪个数据模型
    ...
    {	
    	...
    	{
    		$query ->andFilterWhere(['like','mac_addr',Yii::$app->request->get('mac_addr')]);
    	}

        
        $_page_size = Yii::$app->request->get('page_size') ? Yii::$app->request->get('page_size') : 10;
    	...
    	try{
    		...
    		$_data = $query->offset((Yii::$app->request->get('_page')-1)*$_page_size)->limit($_page_size)->all();
    	}
    	catch(ExceptionNew $e){

    		throw new HttpException(500, 'file not exist', 10002);
    	}
