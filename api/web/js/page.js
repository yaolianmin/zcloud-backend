;(function($,win,doc,unde){
	$.fn.pageFuns = function(options){
		var that = $(this);
		var defaults = {
			pageDiv   : $(this).find("#show_firmware_result"),
		...
		var Method = {
			...
			getData : function(){
				 //$.getJSON(opts.interFace,function(data){
					$(function(){
						var data = opts.interFace;
						//alert("222 : "+JSON.stringify(data));
				    if(data.status == "ok"){
				    	opts.res = data.history_version;
                		...
                		for (var i = 0; i < opts.displayCount; i++) {
							//alert("pageDiv : "+JSON.stringify(opts.res[i]));
                            opts.pageDiv.append(opts.dataFun(opts.res[i],i,opts.displayCount));
                        }

                        for (var i = 0; i < opts.pNum; i++) {
                            opts.pageObj.append(opts.pageFuns(i+1));
                        }
