...
<!--一个弹出的modal中再弹出一个modal的时候，js就会报错“too much recursion”，
从而会影响modal中弹出的那个modal里面的一些jquery插件的使用，
解决方案就是把$.fn.modal.Constructor.prototype.enforceFocus 方法去掉就可以了。
在Bootstap.js 包含以后加上:
-->
</head>
<body>
						...
						<a target="_blank" onmouseover="tip.start(this)" tips="<?=\Yii::t('yii','Delete')?>">
							<?php
								...
								if($power == 1){
									...
									//为了用模态框，所以就放弃表单提交删除，改用ajax提交删除
									$options = ['class' => 'glyphicon glyphicon-trash btn-tool', 'onclick' => 'javascript:return confirm_disp("'.$value.'");'];
								}else
									$options = ['style' => 'display:none;']
...
<?=Html::endForm();?>
		...
		<div id="firmware_info_show" style="margin-left:10%;margin-right:10%;margin-bottom:10%;display:none;">
			...
			<div class="input-group" style="width:80%;margin-left:10%;margin-right:10%;">
				<span class="input-group-addon" style="width:30%;color:#fff;background-color:#337ab7;border-color: #2e6da4;"><?=\Yii::t('yii','FirmwareName');?></span>
			...
			<div class="input-group" style="width:80%;margin-top:10px;margin-left:10%;margin-right:10%">
				...
				<span class="input-group-addon" style="width:30%;color:#fff;background-color:#337ab7;border-color: #2e6da4;"><?=\Yii::t('yii','Upload Attachment');?></span>
			...
			<div class="input-group" style="width:80%;margin-top:10px;margin-left:10%;margin-right:10%;">
				<span class="input-group-addon" style="width:30%;color:#fff;background-color:#337ab7;border-color: #2e6da4;"><?=\Yii::t('yii','DeviceName');?></span>
			...
			<div class="input-group" style="width:80%;margin-top:10px;margin-left:10%;margin-right:10%">
				<span class="input-group-addon" style="width:30%;color:#fff;background-color:#337ab7;border-color: #2e6da4;"><?=\Yii::t('yii','version');?></span>
      ...
      <div class="modal-body">
		...
		<div id="device_Firmware_List" class="file_content" style="display:;">
			...
			<div class="page" >
				...
				<?php
					$options = ['class' => 'pageMenu clearfix', 'style' => 'left:24%'];
					$lang_class = \Yii::$app->session['language'];
					if($lang_class === 'en'){
						$options = ['class' => 'pageMenu clearfix', 'style' => 'left:15%'];
					}
				?>
...
<!------------------------------------------------------------------>				
			<div id="firmware_info_show_2" style="margin-left:10%;margin-right:10%;margin-bottom:10%;display:none;">
				...
				<div class="input-group" style="width:80%;margin-left:10%;margin-right:10%;">
					<span class="input-group-addon" style="width:35%;color:#fff;background-color:#337ab7;border-color: #2e6da4;"><?=\Yii::t('yii','FirmwareName');?></span>
				...
				<div class="input-group" style="width:80%;margin-top:10px;margin-left:10%;margin-right:10%">
					...
					<span class="input-group-addon" style="width:35%;color:#fff;background-color:#337ab7;border-color: #2e6da4;"><?=\Yii::t('yii','Upload Attachment');?></span>
				...
				<div class="input-group" style="width:80%;margin-top:10px;margin-left:10%;margin-right:10%;">
					<span class="input-group-addon" style="width:35%;color:#fff;background-color:#337ab7;border-color: #2e6da4;"><?=\Yii::t('yii','DeviceName');?></span>
				...
				<div class="input-group" style="width:80%;margin-top:10px;margin-left:10%;margin-right:10%">
					<span class="input-group-addon" style="width:35%;color:#fff;background-color:#337ab7;border-color: #2e6da4;"><?=\Yii::t('yii','version');?></span>
...
<script>
		...
		$(".file_content").pageFuns({  /*在本地服务器上才能访问哦*/
			...
			dataFun:function(data,index,displayCount){
				...
				if(power == 1){
					...
					dataHtml += "<button type='button' class='glyphicon glyphicon-trash btn-tool' onclick='javascript:return confirm_disp_2("+JSON.stringify(eachvalue)+");'></button>";
				}
	...
	var show = document.getElementById('show');
	    show.innerHTML="<?= Yii::t('yii','File Management');?>";
